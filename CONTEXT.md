...
- **Acquisition (Acquirer)**: The unified deep acquisition module (`src/core/acquirer.ts`) exposing `Acquirer.acquire()` and `Acquirer.update()`. Owns 100% of repository spec parsing, security assertions (path traversal & git flag injection blocking), git cloning into pristine `repos/`, SHA-256 content hashing, `apm.lock.yaml` manifest management, and `source-registry.json` provenance ledger updates.
