...
- **Acquisition (PackageAcquirer)**: The unified acquisition module that turns any source spec (local directory, git URL, GitHub shorthand `owner/repo#ref`, tree subfolder) into a validated `AcquiredPackage`. Owns ref validation, flag-injection security checks, shallow git cloning into `~/.agentplugins/repos/`, fetch cache management, ephemeral sandbox lifecycles for preview, content hashing, `apm.lock.yaml` generation, and `source-registry.json` provenance ledger updates.
