/**
 * Native TypeScript Schema Validator for OpenAI Codex Plugin Manifests (.codex-plugin/plugin.json).
 * 
 * Enforces the normative Codex validation rules without any dependency on external Python scripts.
 */
...
export function validateCodexManifest(manifest: unknown): CodexValidationResult {
  const errors: string[] = [];
